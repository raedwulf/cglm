...
/*
 Functions:
  ...
  CGLM_INLINE void glm_persp_decomp_z_rh_zo(mat4 proj,
                                            float * __restrict nearVal,
                                            float * __restrict farVal)
  CGLM_INLINE void glm_persp_decomp_far_rh_zo(mat4 proj, float * __restrict farVal)
  CGLM_INLINE void glm_persp_decomp_near_rh_zo(mat4 proj, float * __restrict nearVal)
...
glm_persp_decomp_x_rh_zo(mat4 proj,
                         float * __restrict left,
                         float * __restrict right) {
  float nearVal, m20, m00, m22;

  m00 = proj[0][0];
  m20 = proj[2][0];
  m22 = proj[2][2];

  nearVal = proj[3][2] / m22;
...
CGLM_INLINE
void
glm_persp_decomp_far_rh_zo(mat4 proj, float * __restrict farVal) {
...
CGLM_INLINE
void
glm_persp_decomp_near_rh_zo(mat4 proj, float * __restrict nearVal) {
  *nearVal = proj[3][2] / proj[2][2];
}
...
glm_persp_sizes_rh_zo(mat4 proj, float fovy, vec4 dest) {
  ...
  glm_persp_decomp_z_rh_zo(proj, &nearVal, &farVal);
